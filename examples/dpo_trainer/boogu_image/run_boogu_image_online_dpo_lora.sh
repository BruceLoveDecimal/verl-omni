# Boogu-Image online Diffusion-DPO LoRA, vllm_omni rollout
#
# Prerequisites: same as examples/flowgrpo_trainer/boogu_image/README.md
# (boogu-image package + trust_remote_code).
#
# Online DPO: rollout generates rollout.n candidates per prompt via the
# deterministic ODE with standard Boogu text CFG, the reward model scores
# them, and the trainer pairs top-vs-bottom per prompt for the DPO loss.
...
python3 -m verl_omni.trainer.main_diffusion \
    ...
    actor_rollout_ref.actor.diffusion_loss.dpo_beta=500.0 \
    ...
    trainer.project_name=diffusion_dpo \
    trainer.experiment_name=boogu_image_online_dpo_lora \
