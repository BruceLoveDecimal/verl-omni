...
> **Note.** The Ascend stack is still validated against vLLM 0.24.x and lags
> the CUDA stack (vLLM 0.25.0). The shared vLLM-Omni git pin targets vLLM
> 0.25; NPU users should stay on the last known-good combination until
> `vllm-ascend` catches up.
...
| Extra       | Adds                                                          | When                     |
| ----------- | ------------------------------------------------------------- | ------------------------ |
| `gpu`       | `vllm==0.25.0`, `kernels==0.14.1`, `liger-kernel`             | CUDA rollout + actor FA3 |
| `vllm-omni` | `vllm-omni==0.25.0rc1`                                        | Optional PyPI baseline only; CI/docs use the git pin above |
...
### Installing VeOmni alongside vLLM 0.25.0

VeOmni 0.1.11's `gpu` extra pins `torch==2.9.1+cu129`, which may conflict with the torch version pulled in by `vllm==0.25.0`. A plain `uv pip install veomni[gpu,dit]==0.1.11` therefore fails dependency resolution.
...
If you want VeOmni's full `[gpu,dit]` extras (flash-attn variants, liger-kernel, cuda-python, etc.), install them in a separate environment not pinned to vllm 0.25.0; verl-omni does not need them.
